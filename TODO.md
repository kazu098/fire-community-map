...
## ステップ4: コミュニティ投稿（読んだ本・質問相談・note・お金の相談・介護医療）を連携する

詳細設計はGitHub issue #59を参照。旅行は既存の旅行投稿マップ（場所付き）に集約する方針のため、この機能のスコープからは外している。

- [x] サーバー管理者にF研Botへの対象8チャンネル（`雑談` `質問・相談コーナー` `お金の話・相談` `こんな本読みました` `旅行` `介護・医療` `子育て` `不動産`）の閲覧権限付与を依頼する（`View Channel` / `Read Message History`）→ 既存4チャンネルは付与済み。`子育て` と `不動産` は追加対象
