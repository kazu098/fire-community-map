#!/usr/bin/env python3
"""Seed member_profiles/member_tags/member_links for the tenth tag-display batch (1 member: Aki).

Aki collides in Discord display name with an unrelated earlier member ("晶(aki)"),
so the self-intro was picked manually by matching content against the master
profile spreadsheet (ワインバー経営 in 世田谷/葉山, note.com/ingalleon) rather than
via fetch_self_intros.py's automatic display-name match.

Same upsert pattern as load_member_profiles.py / load_member_profiles_batch2.py / batch3 / batch4 / batch5 / batch6 / batch7 / batch8 / batch9.
"""
...
MEMBER_LINKS: dict[str, list[dict[str, str]]] = {
    "Aki": [{"label": "note", "url": "https://note.com/ingalleon"}],
}

# category is one of: investment_style, fire_status, mbti, skill, interest, affiliation
MEMBER_TAGS: dict[str, dict[str, list[str]]] = {
    "Aki": {
        "investment_style": ["不動産投資", "米国株式", "国内株式", "インデックス投資", "米国債", "暗号資産", "ロボアド"],
        "fire_status": ["FIRE済"],
        "mbti": [],
        "skill": ["ワインバー経営", "ワイン造り", "事業開発"],
        "interest": ["ワイナリー巡り", "バー巡り", "旅行", "世田谷/葉山二拠点生活"],
    },
}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Seed batch 5 of member_profiles/member_tags/member_links.")
    parser.add_argument("--env-file", default=".env")
    parser.add_argument("--self-intros", default="tmp/tag_display_self_intros_batch10.json")
