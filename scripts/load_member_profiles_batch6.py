#!/usr/bin/env python3
"""Seed member_profiles/member_tags/member_links for the sixth tag-display batch (2 members: まるこ, Sal).

Same upsert pattern as load_member_profiles.py / load_member_profiles_batch2.py / batch3 / batch4 / batch5.
"""
...
MEMBER_TAGS: dict[str, dict[str, list[str]]] = {
    "まるこ": {
        "investment_style": ["つみたてNISA"],
        "fire_status": ["サイドFIRE"],
        "mbti": [],
        "skill": [],
        "interest": ["写真撮影", "編み物", "ハーブ栽培", "アロマテラピー", "犬"],
    },
    "Sal": {
        "investment_style": ["投資信託の取り崩し(S&P500)", "日本高配当株"],
        "fire_status": ["FIRE済"],
        "mbti": [],
        "skill": ["ゲーム開発(Unity)", "バス運転士"],
        "interest": ["移住(沖縄)", "旅行", "ウォーキング"],
    },
}
...
def main() -> int:
    ...
    parser.add_argument("--self-intros", default="tmp/tag_display_self_intros_batch6.json")
