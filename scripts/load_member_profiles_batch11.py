#!/usr/bin/env python3
"""Seed member_profiles/member_tags/member_links for the eleventh tag-display batch (2 members: Hiro, あじ).

Hiro's self-intro text declares nickname "Taka", not "Hiro" -- the automatic
matcher picked it via exact Discord display-name match ("Hiro"). Content
(アラフォー/東京/会社員/投資信託オルカン/5年後FIRE) matches the master profile
spreadsheet's "Hiro" row, so this looks like a later nickname change rather
than a mismatch; used as-is.

Same upsert pattern as load_member_profiles.py / load_member_profiles_batch2.py / batch3 / batch4 / batch5 / batch6 / batch7 / batch8 / batch9 / batch10.
"""
...
MEMBER_TAGS: dict[str, dict[str, list[str]]] = {
    "Hiro": {
        "investment_style": ["投資信託(オルカン)"],
        "fire_status": ["FIRE目指し中"],
        "mbti": [],
        "skill": ["デザイナー"],
        "interest": ["テニス", "ジャズピアノ", "海外語学留学"],
    },
    "あじ": {
        "investment_style": ["外貨建て保険", "NISA", "iDeCo"],
        "fire_status": ["サイドFIRE"],
        "mbti": ["INTP-A"],
        "skill": ["漫画家"],
        "interest": ["交流", "アウトドア", "ゲーム"],
    },
}
...
def main() -> int:
    ...
    parser.add_argument("--self-intros", default="tmp/tag_display_self_intros_batch11.json")
