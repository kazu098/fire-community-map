#!/usr/bin/env python3
"""Seed member_profiles/member_tags/member_links for the thirteenth tag-display batch (1 member: ドロリッチ).

Same upsert pattern as load_member_profiles.py / batch2-12.
"""
...
MEMBER_LINKS: dict[str, list[dict[str, str]]] = {
    "ドロリッチ": [
        {"label": "note", "url": "https://note.com/u041757h"},
    ],
}
...
MEMBER_TAGS: dict[str, dict[str, list[str]]] = {
    "ドロリッチ": {
        "investment_style": ["日本株(優待メイン)", "米国株(ETFメイン)", "投資信託(オルカン)"],
        "fire_status": ["バリスタFIRE目指し中"],
        "mbti": ["INTJ-A"],
        "skill": ["ICT教材開発", "プログラミング講師"],
        "interest": ["将棋", "サウナ", "料理"],
    },
}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Seed batch 13 of member_profiles/member_tags/member_links.")
    parser.add_argument("--env-file", default=".env")
    parser.add_argument("--self-intros", default="tmp/tag_display_self_intros_batch13.json")
