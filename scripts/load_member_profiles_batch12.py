#!/usr/bin/env python3
"""Seed member_profiles/member_tags/member_links for the twelfth tag-display batch (1 member: APi).

The tag-display response sheet nickname is "AP", but the Discord self-intro
and master profile spreadsheet both use "APi", which also matches the
existing member_locations map pin -- used "APi" as the canonical nickname.

Same upsert pattern as load_member_profiles.py / batch2-11.
"""
...
MEMBER_TAGS: dict[str, dict[str, list[str]]] = {
    "APi": {
        "investment_style": ["インデックス投資(オルカン)"],
        "fire_status": ["セミFIRE済み"],
        "mbti": [],
        "skill": ["個人事業主(経営)"],
        "interest": ["海外旅行", "国内旅行", "交流"],
    },
}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Seed batch 12 of member_profiles/member_tags/member_links.")
    parser.add_argument("--env-file", default=".env")
    parser.add_argument("--self-intros", default="tmp/tag_display_self_intros_batch12.json")
