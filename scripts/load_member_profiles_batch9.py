#!/usr/bin/env python3
"""Seed member_profiles/member_tags/member_links for the ninth tag-display batch (1 member: ノコ).

Same upsert pattern as load_member_profiles.py / load_member_profiles_batch2.py / batch3 / batch4 / batch5 / batch6 / batch7 / batch8.
"""
...
MEMBER_LINKS: dict[str, list[dict[str, str]]] = {
    "ノコ": [
        {"label": "note", "url": "https://note.com/noco_retired2022"},
        {"label": "世界一周旅行の作り方", "url": "https://amzn.to/3OAwtPQ"},
        {"label": "世界一周旅行の作り方〜旅のフォトブック", "url": "https://amzn.to/48b96D2"},
    ],
}
...
MEMBER_TAGS: dict[str, dict[str, list[str]]] = {
    "ノコ": {
        "investment_style": ["株式投資", "旧NISA満額投資"],
        "fire_status": ["FIRE済"],
        "mbti": ["INTP-A"],
        "skill": ["マイクロ法人運営", "Kindle出版"],
        "interest": ["旅行(国内外)", "写真", "東京/長野二拠点生活"],
    },
}
...
def main() -> int:
    ...
    parser.add_argument("--self-intros", default="tmp/tag_display_self_intros_batch9.json")
