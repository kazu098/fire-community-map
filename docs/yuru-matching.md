...
1. メンバー詳細画面で「誰として表示中か」を確認する。
2. 自分のプロフィールで「マッチングに参加する」をONにする。
3. マッチング頻度(2日に1回/3日に1回/週1回/隔週/月1回)を選ぶ。
4. 曜日×時間帯(午前/午後/夜)の○×をクリックして、毎週の空き時間を登録する。
5. 毎週のパターンと違う日だけ、必要に応じて「特定日の例外」から1時間単位で空き/予定ありを登録する。
6. 条件が合うメンバーが見つかると、BotがDiscordのゆるマッチングチャンネルに紹介投稿する。
7. 紹介投稿後はDiscordスレッド内で日程を相談する。Botが候補日を検知できた場合は確認投稿を出し、参加メンバーの確認後に開催決定を投稿する(詳細は[ゆるマッチングの日程調整・一時通話部屋](./yuru-matching-scheduling.md)を参照)。
