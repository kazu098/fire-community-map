...
- **ニュース**: `scripts/topic_news.py`が、タグの値でGoogleニュースRSS検索を信頼できる報道系サイト(NHK・日経・Reuters・東洋経済オンライン・ダイヤモンドオンライン)に絞って叩く。投資助言に見える出所不明な記事を避けるための絞り込みで、該当がなければ単に空リストとする(一般ニュース検索へのフォールバックはしない)。`topic_news_cache`に14日キャッシュし、同じタグの再取得を避ける(ゆるマッチングの開催間隔は概ね1か月以内のため、鮮度よりキャッシュの手軽さを優先)。ニュース対象は`NEWS_ELIGIBLE_CATEGORIES`(興味・興味あるテーマや話題・投資スタイル・活動/部活)のみで、MBTIやFIREタイプのような短い記号的な値は対象外。
