...
- ドライブのファイル名にあるペンネームと、コミュニティのメンバー名(nickname)が異なる場合がある(例: 「エキストラ道」のファイル名は「浮雲」だが著者表記は「浮影」)。`member_nickname`は`member_profiles`と一致する方を使い、`author_name`は本に実際に記載されている表記を使う
- Google Driveフォルダの一覧は、このリポジトリで使うGoogle Drive MCP接続のアカウントでは全件が見えないことがある(フォルダのオーナーと接続アカウントが別のため)。全件を確認する場合は実ブラウザでフォルダを開き、ページ内の`[data-id]`属性からファイルIDを取得する
