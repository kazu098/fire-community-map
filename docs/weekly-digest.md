...
- 1チャンネルにつき最大1件(`max_per_channel`)まで。雑談のように投稿数が多いチャンネルはリアクション数も自然と稼ぎやすく、対策なしだと上位互換枠を独占して他の約28チャンネルが一切表に出てこなくなるため、まず「各チャンネルの最上位1件」を優先的に埋め、それでも枠が余った場合だけ同じチャンネルから2件目を拾う
- 表示文字数は280文字(`display_limit`)まで許容し、それを超える場合だけ文末(。！？)で切る(`trim_to_sentence`)。以前は140文字で切っていたため「あれ？」のようなオチの手前で切れて要約になっていない状態だったが、実データでは上位の投稿はほぼ300文字以内に収まっていたので上限を引き上げて解消した
